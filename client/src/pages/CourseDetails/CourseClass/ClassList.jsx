...
export default function ClassList({ courseId, classCodeStudent }) {
  ...
  const fetchClasses = async () => {
    try {
      ...
      if (currentUser.Role === "teacher") {
        res = await axios.get(`/classes/${courseId}`);
        setClasses(res.data);
        setIsTeacher(true);
      } else if (currentUser.Role === "student") {
        res = await axios.get(`/classes/${courseId}/class/${classCodeStudent}`);
  ...
  const fetchClassStudent = async (classCode) => {
    try {
      const res = await axios.get(`/classes/${courseId}/class/${classCode}`);
      setClassStudent(res.data);
    } catch (error) {
      console.error(error);
    }
